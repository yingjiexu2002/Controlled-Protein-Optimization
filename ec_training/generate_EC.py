...
def predict_fn(model,inputs,past=None):
    with torch.no_grad():
        ...
        # remove non-AA token logits，只保留AA-token和stop-token，其中stop-token的ctrl代码为503
        output = torch.cat([output[:,:,-26:-1],output[:,:,503:504]],2) 
        return output, past
...
def parser_labels(label, mapping_files):
    ...
    parts = label.split('.')
    # 使用列表推导式创建新列表
    label_list = [f'{i+1}_{part}' for i, part in enumerate(parts)]
    result=[]
    for i in label_list:
...
def main():
    ...
    with open('mapping_files/ec_level_to_ctrl.p','rb') as handle:
    ...
    for i in range(0,args.num_sample_batches):
        ...
        ctrl_idx_to_aa[503] = "#"     # ctrl_code=5，表示STOP标记
