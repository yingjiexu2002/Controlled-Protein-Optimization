...
from EC_transformProtein_v2 import transformProtein
...
#***************参数解析********************
parser = argparse.ArgumentParser(description='EC number finetuning code')
...
parser.add_argument('--local-rank', default=0, type=int, help='node rank for distributed training')  # 多卡训练
...
class Trainer(object):
    def __init__(self, model, warmup_iteration, seq_length, batch_size, num_workers, vocab_size, model_dir, save_iter):
        ...
        self.transformFull = transformProtein(maxSampleLength=seq_length + 1, dropRate=0.0, noflipseq=True)  # 完整转换，不反转序列

        self.writer = SummaryWriter("logs/log0")   # from tensorboard
    ...
    def train(self, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            for i, (sample, labels, existence, padIndex, begAAindex) in enumerate(dataloader):
                ...
                # 保存模型
                if (iter_num) % self.save_iter == 0 and local_rank == 0:
                    # torch.save({'epoch': epoch, 'chunknum': chunknum, 'iteration': i,
                    #             'model_state_dict': self.model.state_dict(),
                    #             'optimizer_state_dict': self.optimizer.state_dict(),
                    #             'loss': loss,
                    #             }, self.model_dir)
                    # print("i = "+str(i)+" 时，模型保存为 "+ str(self.model_dir) )
                    ...
                    torch.save({'model_state_dict': lora.lora_state_dict(model.module)}, lora_model_path)
            loss_e /= num_e     # 计算每个epoch的平均损失值

        if local_rank==0:
          print("保存最终的主模型")
          checkpoint_path = os.path.join(args.model_dir, f'EC_finetuned_with_lora.pt')
