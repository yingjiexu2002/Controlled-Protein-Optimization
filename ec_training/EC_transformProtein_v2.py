...
class transformProtein:
    ...
    def __init__(self, mapfold = 'mapping_files/', maxSampleLength = 512, 
        ...
        # 打开标签映射pickle文件
        with open(os.path.join(mapfold,'ec_level_to_ctrl_v2.p'),'rb') as handle:
    ...
    def transformSample(self, proteinDict, justidx = True):
        ...
        # 随机遮盖末尾的标签，以便提升模型的泛化能力
        # 从1到4之间随机选择一个数
        label_num = random.randint(1,4)
        # 选择taxa的前label_num个标签
        taxa = taxa[:label_num]
        # 剩下的位置用"none"填充
        while len(taxa)<4:
            taxa.append('none')
...
# # 测试函数功能是否正常
# with open('train_data/train_data_EC1=2_v2.p', 'rb') as handle:     # pklpath：包含蛋白质数据的pickle文件的路径
#     train_chunk = pickle.load(handle)
# uid = 0
# obj = transformProtein(dropRate = 0.0, noflipseq = True)
