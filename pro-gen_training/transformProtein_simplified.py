...
class transformProtein:
    ...
    def __init__(self, mapfold = 'mapping_files/', maxSampleLength = 512, selectSwiss = 1.0,
                 ...
                 maxKwPerSample = 5, dropRate = 0.0, noflipseq = False):
        #类内局部变量的初始化
        ...
        with open(os.path.join(mapfold,'familyname_to_code.p'),'rb') as handle:
            self.familyname_to_code = pickle.load(handle)
    ...
    def transformSample(self, proteinDict, justidx = True):
        ...
        kws = {}
        # taxa标签为家族名称
        taxa = proteinDict['other_taxa']
        # 以一定的概率翻转序列
        seq = self.transformSeq(proteinDict['seq'])

        code_PAD = self.familyname_to_code['PAD']
        code_stop = self.familyname_to_code['STOP']
        ...
        if justidx: # 默认为True
            ...
            for tax_line in taxa:
                encodedSample.append(self.familyname_to_code[tax_line])
            seq_idx = 0
            # 将氨基酸序列进行编码
